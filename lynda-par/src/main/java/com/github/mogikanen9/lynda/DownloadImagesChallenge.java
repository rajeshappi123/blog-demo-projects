...
class ParallelImageDownloader {
    ...
    public int downloadAll() {
        ...
        for (int num : imageNumbers) {
            futures.add(exec.submit(new DownloadTask(num)));
        }
        // totalBytes += downloadImage(num);
        int rs = futures.stream().map((future) -> {
            try {
                return future.get();
    ...
    class DownloadTask implements Callable<Integer> {
        ...
        public DownloadTask(int imageNumber) {
...
public class DownloadImagesChallenge {
    ...
    public static void main(String[] args) {
        ...
        final int[] IMAGE_NUMS = IntStream.rangeClosed(1, 20).toArray(); // images to download
